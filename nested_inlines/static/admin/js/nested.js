...
(function($) {
	...
	// Tabular inlines ---------------------------------------------------------
	$.fn.nestedTabularFormset = function(options) {
	...
	// Stacked inlines ---------------------------------------------------------
	$.fn.nestedStackedFormset = function(options) {
		var $rows = $(this);
	...
	function create_nested_formsets(parentPrefix, rowId) {
		...
		nested_inlines.each(function(index) {
			...
			if (isTabular) {
				var formset = template.find('.tabular.inline-related tbody tr.' + formset_prefix + '-not-nested').nestedTabularFormset(options);
				...
				var formset = template.find(".inline-related").nestedStackedFormset(options);
